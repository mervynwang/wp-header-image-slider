...
function boom_get_header_images() {
	...
	if( get_theme_mod( 'header_image', '' ) == 'boom-slider-uploaded' ) {
		...
		// /home2/axlovisi/public_html/wp-content/plugins/header-image-slider/general-template.php
		$dir = './wp-content/uploads/header_images/';
		$available_headers = array();
		if ($list = scandir($dir)) {
			foreach($list AS $file){
				if(preg_match('/[\w-_]+\.(jpg|jpeg|png|gif)$/', $file)) $available_headers[] = 'http://axlovision.com/wp-content/uploads/header_images/' .$file;
			}
		}
